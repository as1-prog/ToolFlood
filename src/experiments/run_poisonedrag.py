...
def generate_attacker_tools_for_domain(
    ...
    train_queries, test_queries = limit_queries(
        train_queries,
        test_queries,
        poisonrag_cfg.max_train_queries,
    ...
    # Run PoisonRAG attack on train queries (generate poisoned tools)
...
async def run_experiment_for_model(
    ...
    asr = calculate_asr(
        eval_results["successful_selections"],
...
async def _run_single_experiment(cfg_path: Path, models_path: Path) -> int:
    ...
    if not generator_model_name:
        ...
        generator_model_name = poisonrag_cfg.victim_model
        logger.info(
    ...
    # Determine grids to process (prefer experiment config fields, but fall back
    # to poisonrag_attack single-model fields for backwards compatibility).
    attack_embedding_model_names = (
        exp_cfg.attack_embedding_models
        if getattr(exp_cfg, "attack_embedding_models", None)
        else [poisonrag_cfg.embedding_model]
    )
    victim_embedding_model_names = (
        exp_cfg.victim_embedding_models
        if getattr(exp_cfg, "victim_embedding_models", None)
        else [poisonrag_cfg.victim_embedding_model]
    )
    victim_model_names = (
        exp_cfg.victim_models
        if getattr(exp_cfg, "victim_models", None)
        else [poisonrag_cfg.victim_model]
    )
